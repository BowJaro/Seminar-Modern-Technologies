...
def character_recog_tesseract(img):
    ...
    text = pytesseract.image_to_string(img, config='--psm 10 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
    return text.strip()
...
def extract_and_compare_text(image_path):
    ...
    # pattern = r'^\d{2}[A-Za-z]\d{5,6}$'  # 2 numbers + 1 letter + 5 or 6 numbers

    # # Create counters for character frequency
    # method1_count = Counter(method1_text)
    # method2_count = Counter(method2_text)

    # # Compare results with the pattern
    # method1_valid = re.match(pattern, method1_text)
    # method2_valid = re.match(pattern, method2_text)

    # if method1_valid and method2_valid:
    #     # Both matched, return the most common character count from both
    #     combined_count = method1_count + method2_count
    #     most_common_text = ''.join([char * count for char, count in combined_count.most_common()])
